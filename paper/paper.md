...
The RC5 model has been chosen as temporal transition algebra because it offers five exhaustive and mutually exclusive relations [@randell1992]: unchanged (EQ), grown (PP), shrunk (PPi), partially reorganized (PO) and disconnected (DC). With this transition vocabulary, a neuroscientist can read the meaning directly from its visualization and a subgraph mining algorithm can operate on its finite states. However, the magnitude of the change is lost: for instance, gaining one area or gaining twenty areas both yield the same PP label. An example is illustrated in \autoref{fig:concept}(C).
