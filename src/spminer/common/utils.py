...
def wl_hash(g, dim=64, node_anchored=False):
    ...
    vecs = np.zeros((len(g), dim), dtype=np.int64)  # Changed to int64
    if node_anchored:
        for v in g.nodes:
    ...
    for i in range(len(g)):
        newvecs = np.zeros((len(g), dim), dtype=np.int64)  # Changed to int64
        for n in g.nodes:
            newvecs[n] = vec_hash(np.sum(vecs[list(g.neighbors(n)) + [n]],
                axis=0))
        vecs = newvecs
    # Use Python's hash instead of raw numpy sum to avoid overflow
    return hash(tuple(np.sum(vecs, axis=0).tolist()))
